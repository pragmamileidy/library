...
function set_checked_by_ticket(ticket, isChecked)
{
  $.each(show_tickets, function(index, item) {
    ...
    var checkboxes = $("input[key='Pagar boleta']");// VDIAZ $(fields[0]).find("input");
...
$.each(show_tickets, function(index, item) {
  ...
  if (ticket_pos === -1)
  {
    ...
    label_str    += "<h3>Nro. de Boleta: " + ticket_num                       + " </h3> ";
    label_str    += "<div   ><h2>Monto Total:"    + format1(parseFloat($(fields[8]).find("input").val()),"") + "</h2></div>";
    ...
    if (flagp_tickets[ticket_pos])
    {
      ...
      var label_str = ""; // <input  type='checkbox' class='groupHeader' " + check +"/> 
      label_str    += "<h3>Nro. de Boleta: <b>" + ticket_num                       + "</b></h3> ";
      label_str    += "<h2>Monto Total: "    + format1(total_ticket,"") + "</h2>";
      label.html(label_str);

      flagp_tickets[ticket_pos] = false;
    } else {
      label.html("");
      $("input[key='Pagar boleta']").css("display","none");// VDIAZ  select checkboxes
    }
...
$(document).ready(function(){
  ...
  $('input[value=Próximo]').attr("style", "background: #aeb404; height: 40px; width: 150px;font-size: 16px;text-align: center; border-radius: 10px;");

  // Modifica el nombre del botón próximo a "PAGAR"
  $('input[value=Próximo]').val("PAGAR");
  ...
  $( "#check-all" ).on( "change", function() {
      if ($(this).prop('checked')){    
        $("input[key='Pagar boleta']:checked").click();  
        $("input[key='Pagar boleta']").click();    
      }else{         
        $("input[key='Pagar boleta']:checked").click(); 
      }
      //$("input[key='Pagar boleta']").click();
  ...
  //    Handler para reinyectar el texto cuando cambie el icono del botón  
  $(".panel-control-collapse").on( "click", function() {
     console.log($(this).html());
     $("<span> Ocultar detalle</span>").prependTo($(this).children(".ui-icon-minusthick").parent());
     $("<span> Ver detalle</span>").prependTo($(this).children(".ui-icon-plusthick").parent());
  });
          
          
  //Para cada boleta, cambiar presentación en html
  $.each( boletas, function(k, v) {    
    var check = $(v).find("input[type='checkbox']");
    var panel = $(v).find('.panel-title');
    
    //se mueve el checkbox
    $(check).parent().parent().clone().appendTo($(panel));
  ...
  $(".part-box .label2").attr("style","background-color: #f1efef;");


  var montoGlobal = $("#field_boletas_por_pagar_monto_monto_de_la_multa").html();
  console.log("monto",montoGlobal);        
  $("#field_boletas_por_pagar_monto_monto_de_la_multa").html("<h2>"+montoGlobal+"</h2>");          
  //se agrega modal
  /*
