...
#:::: DV: 25/04/2017 Modificado por case el tipo de documento
...
form_data.set("boletas_por_pagar.monto.mensaje_notificacion","")
# AGREGADO 26-05-2017
...
# AGREGADO 26-05-2017
if Hash === result && Array === result["response"]
  if result["response"].empty?
    ...
    result["response"].each_with_index do |data, index|
      ...
      data["infFaltas"].each do |falta|
        monto_falta = falta["infMontoTotal"].to_f - falta["infMontoDesacato"].to_f
        ...
        faltas += "<tr><th>Monto Falta: </th> <td>#{sprintf('%.2f', monto_falta)}</td></tr>"
  ...
  transition_errors << I18n.t("views.notifications.notifications.error")
end
# AGREGADO 26-05-2017
...
if valor == "Electrónico"
  form_data.set("formas_de_pagos_generico.pago_electronico_mensaje.tipo_de_pago_electronico", "Pago Electrónico")
    ...
    on_transition do
      #::: DV: Validar se chequee la tabla para pagar el monto de la boleta :::#
...
#:::::::::::: DV: ONTRANSITION  PASO 2 17/04/2017 :::::::::::::::#
form_id = @task["form"]
...
if result["response"].present? && result["response"] == true
...
#:::::: DV: Invocar el MakePayment :::::::::#
#:::::::::::: CODIGO PARA INVOCAR get_payment_status :::::::::#
#:::::: se usa para generar nuevo pago sólo si el status es "PaymentNotCompleted" :::::::::#
