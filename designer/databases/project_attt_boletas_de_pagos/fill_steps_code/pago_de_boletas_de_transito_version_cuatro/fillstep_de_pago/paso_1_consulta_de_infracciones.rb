  ...
  class Paso1ConsultaDeInfracciones < TemplateCode::Step
    ...
    on_becoming do
      if form_data.get("es_deudor_o_no.datos_de_la_persona.tipo_documento").blank?
...
if (infraccion_propia.blank?)
    ...
    if @owner["kind_of_user_type"] == "NaturalPerson"
        if _person == _owner
            ...
            form_data.set("es_deudor_o_no.datos_de_la_persona.cedula",numdoc) if tipodoc == "Cédula"
            form_data.set("es_deudor_o_no.datos_de_la_persona.dato_a_consultar",numdoc) if tipodoc == "Pasaporte"
## Chantal Valverde Seteando dato_a_consultar con el pasaporte del solicitante            
...
nro_doc = form_data.get("es_deudor_o_no.datos_de_la_persona.dato_a_consultar")
nro_ced = form_data.get("es_deudor_o_no.datos_de_la_persona.cedula")

if nro_doc.blank? && nro_ced.blank?
    transition_errors << "El dato a consultar es requerido, por favor verifique"
end
    end
...
//::: DV: Limpiar nro de documento cuando se cambia de tipo ::://
$(document).ready(function(){
