  ...
  class Paso2InformacionDeBoletasPorPagar < TemplateCode::Step

    on_becoming do
      if @owner["kind_of_user_type"] == "NaturalPerson"
    tipo_doc = form_data.get("datos_de_la_persona.datos_de_la_persona.tipo_documento_persona_natural")
    tipodDoc = (tipo_doc == "Cédula") ? "CIP" : "PAS"
    nro_doc  = form_data.get("datos_de_la_persona.datos_de_la_persona.nro_de_documento")
else
   tipodDoc = "RUC"
   nro_doc  = form_data.get("datos_de_la_empresa.datos_de_la_empresa.nro_de_ruc")
end
...
campoOcuto = form_data.get("boletas_por_pagar.boletas.campo_oculto")
respuesta = result["response"]
#Si trae data de boletas, llenar la sección de boletas por pagar
if respuesta.blank?
    form_data.set("boletas_por_pagar.resultado_de_la_consulta.resultado_de_la_consulta","Usted no tiene multas pendientes. Avance al próximo paso.")
    form_data.set("boletas_por_pagar.boletas.monto_de_la_multa","0")
    form_data.set("boletas_por_pagar.boletas.motivo_de_la_multa","false")
else
    form_data.set("boletas_por_pagar.resultado_de_la_consulta.resultado_de_la_consulta","Usted tiene boleta(s) pendiente(s) por pagar, revise la información del pago y avance al próximo paso. ")
    if campoOcuto.blank?
    form_data.set("boletas_por_pagar.boletas.campo_oculto", result)
    end
    form_data.set("boletas_por_pagar.boletas.motivo_de_la_multa","true")
end
...
$(document).ready(function(){
  ...
  function build_grib(elements, table_name){
    ...
    column_names = ['Seleccione','childrens'];
    grid_data = [];
    models = [
                {name:'Seleccione',index:6,sortable:false,editable:true,edittype:'checkbox', hidden: false, classes: 'cbox',editoptions:{ value:"true:false"}, formatter: "checkbox", formatoptions: {disabled : true}},
        ...
        $.each(grid_data[pos], function(key,value){
          ...
          if($.inArray(column_name,column_names) == -1){
            ...
            if ($.inArray(column_name, new Array("Monto Boleta", "Monto Desacato", "Monto Total")) !== -1) {
              models.push({name:column_name,index:column_name.length,sortable:false,editable:false,edittype:'text', align:'right'}); //, sorttype:"float", align:'right', summaryTpl: "{0}", summaryType: "sum"
            } else {
      ...
      */
      ...
      if (grid_data.length != 0){
         $("#grid").jqGrid({
            ...
            height: 150,
            rowNum: 800,
            colNames: ['', '','Nro de Placa', 'Nro Boleta', 'Fecha', 'Motivo', 'Imposición', 'Monto Boleta', 'Monto Desacato', 'Monto Total'],
            multiboxonly: true,
            altRows: true,
            colModel: models,
            viewrecords: true,
            ...
            groupingView : {
              ...
              groupText: [group_count_text("{0}", "{childrens}")], 
              groupSummary: [true],
              groupCollapse: true
            },
...
function group_count_text(nro_boleta, nro_rows)
{
  
  return "<input  type='checkbox' class='groupHeader' checked='checked'/>" + 
         "<strong> Nro. de Boleta:  " + nro_boleta + " - Faltas: " + nro_rows + " </strong>";
