  ...
  class Paso0ConfirmacionDeAprobacionDeEntidades < TemplateCode::Step
...
entidades.each_index do |i|
	if entidades[i] && (entidades[i]["siglas"].to_s.upcase == _siglas)
		...
		# Inicializacion de datos del agente - LG
		form_data.set("entidad_evaluadora.funcionario_de_entidad_evaluadora_#{i}_de_2do_nivel.usuario", _agente["name"])
    ...
    on_transition do
      #::: OBSERVACIONES 03 JULIO 2015 ENTIDAD EVALUADORA NO DEBE ENVIAR A REPARO SIN LA OBSERVACION

(0..14).each do |_num|
	eval = form_data.get("entidad_evaluadora.funcionario_de_entidad_evaluadora_#{_num}_de_2do_nivel.evaluacion_ok_reparo")
	obs_re = form_data.get("entidad_evaluadora.funcionario_de_entidad_evaluadora_#{_num}_de_2do_nivel.observaciones")
		if (eval == "REPARO" and obs_re.blank? and eval != "OK")
