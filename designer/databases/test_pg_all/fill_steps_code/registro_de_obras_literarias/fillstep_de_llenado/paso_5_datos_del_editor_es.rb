  ...
  class Paso5DatosDelEditorEs < TemplateCode::Step
...
if (tipo_obra == "Publicada")
    ...
    (1..5).each do |_num|
                    nombre2 = form_data.get("datos_de_los_editores_instance_#{_num}.datos_de_la_persona.primer_nombre")
                    nombre3 = form_data.get("datos_de_los_editores_instance_#{_num}.datos_de_la_persona.segundo_nombre")
                    nombre4 = form_data.get("datos_de_los_editores_instance_#{_num}.datos_de_la_persona.primer_apellido")
                    nombre5 = form_data.get("datos_de_los_editores_instance_#{_num}.datos_de_la_persona.segundo_apellido")
                    nombre6 = form_data.get("datos_de_los_editores_instance_#{_num}.datos_de_la_persona.apellido_de_casada")
                    
                    nombre_completo = nombre2.to_s + " " + nombre3.to_s + " " + nombre4.to_s + " " + nombre5.to_s + " " + nombre6.to_s
                    
                    form_data.set("datos_de_los_editores_instance_#{_num}.datos_de_la_persona.nombre_completo", nombre_completo)
    end
    ...
    AJAX_CALLS <<-STEPAJAXCODE 
      /////////////////////MT - HABILITA CAMPOS SI ES PASAPORTE, ESCONDE PARTES DEPENDIENDO DEL TIPO DE DOCUMENTO
...
$(document).ready(function(){
    ...
    if ($(dato).val() != ''){
	    ...
	    var productor = "#field_datos_de_los_editores";
...
// CV 29-Mar	        $(identificacion).removeAttr("disabled");
