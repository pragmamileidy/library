  ...
  class Paso2DatosDelLocalOEmpresa < TemplateCode::Step

    on_becoming do
      if @owner["kind_of_user_type"] == "NaturalPerson"
    form_data.set("solicitante_natural.datos_del_local.se_dedicara_a_ofrecer","Préstamos Prendarios")
else
     form_data.set("solicitante_juridico.datos_generales_de_empresa.se_dedicara_a_ofrecer_texto","Préstamos Prendarios")
end
    end
    ...
    documento_vicepre = form_data.get("miembros_de_la_junta_directiva.vicepresidente.cedula_o_pasaporte")
    if documento_vicepre != "Pasaporte"
        #VERIFICACIÒN NUMERO DE CEDULA DEL PRESIDENTE DE LA JUNTA DIRECTIVA NO PERTENEZCA A UN DIFUNTO
        vicepre = form_data.get("miembros_de_la_junta_directiva.vicepresidente.cedula")   
        resultVicepre = WebserviceConsumer.get(URI.escape('/service/bdin_tribunal_electoral/find.json?cedula=' + "#{vicepre}")).parsed_response
        datosVicepre = resultVicepre["response"]
        if datosVicepre.blank? 
            transition_errors << "La Cédula del Vicepresidente no fue encontrada. Por favor, verifique." 
        else            
            difuntoVicepre = datosVicepre["cod_mensaje"]
            if (difuntoVicepre == "534")
                transition_errors << "No puede continuar con el trámite, ya que el nro. de documento #{vicepre} corresponde a un ciudadano Difunto."
            end
        end
    end
